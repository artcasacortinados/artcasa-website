...
const TapetesPage = () => {
  ...
  const nextImage = () => {
    setCurrentImageIndex((prev) => (prev + 1) % tapetesItems.length)
  }
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="relative py-24 bg-gradient-to-br from-gray-50 to-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-xl text-gray-700 leading-relaxed font-medium max-w-3xl mx-auto">
            Tapetes premium que complementam qualquer decoração. Desde peças clássicas a designs modernos, temos a solução perfeita para cada ambiente da sua casa.
          </p>
        </div>
      </section>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {tapetesItems.slice(0, Math.min(8, tapetesItems.length)).map((image, index) => (
              <div key={index} className="group cursor-pointer" onClick={openGallery}>
                <div className="bg-white rounded-lg border-2 border-gray-200 hover:border-black shadow-sm hover:shadow-lg transition-all overflow-hidden">
                  <div className="h-64 overflow-hidden relative">
                    ...
                    />
                    {index === 7 && tapetesItems.length > 8 && (
                      <div className="absolute inset-0 bg-black bg-opacity-75 flex items-center justify-center">
                        <span className="text-white text-lg font-bold">
                          +{tapetesItems.length - 8} Mais Imagens
                        </span>
                      </div>
                    )}
                  </div>
      ...
      <CallToAction productName="nos nossos tapetes" />
